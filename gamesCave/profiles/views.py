...
def editUserDataView(request):
    if request.method == 'POST':
        ...
        if nick:
            if Profile.objects.filter(nick=nick).exclude(user=request.user).exists():
                msg.append("Profil niezaktualizowany - podany nick jest już zajęty")
            else:
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            if nick:
                updated_profile_data.nick = nick
